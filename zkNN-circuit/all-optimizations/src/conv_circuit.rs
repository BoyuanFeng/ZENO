...
fn conv_kernel_remainder_helper_u8(
    ...
    let length = num_channels * kernel_size * kernel_size;
    let mut idx = 0;
    ...
    for i in 0..num_channels {
        ...
        for j in h_index..(h_index + kernel_size) {
            for k in w_index..(w_index + kernel_size) {
                ...
                mul_tmp1[idx] = constant_mul_cs_helper_u8(cs.clone(), x[i][j][k], kernel[i][j - h_index][k - w_index]);
                mul_tmp2[idx] = constant_mul_cs_helper_u8(cs.clone(), x[i][j][k], kernel_zeropoint);
                mul_tmp3[idx] = constant_mul_constant_cs_helper_u8(
                    cs.clone(),
                    kernel[i][j - h_index][k - w_index],
                    x_zeropoint,
                );
                mul_tmp4[idx] = constant_mul_constant_cs_helper_u8(cs.clone(), x_zeropoint, kernel_zeropoint);;
                idx += 1;
            }
        }
    }
    ...
    for i in 0..num_channels {
        ...
        for j in h_index..(h_index + kernel_size) {
            for k in w_index..(w_index + kernel_size) {
                ...
                mul_tmp1[idx] = x[i][j][k].clone().mul(w_const.clone());
                mul_tmp2[idx] = x[i][j][k].clone().mul(kernel0_const.clone());
                mul_tmp3[idx] = w_const.clone().mul(input0_const.clone());
                mul_tmp4[idx] = input0_const.clone().mul(kernel0_const.clone());
                idx += 1;
            }
        }
    }
