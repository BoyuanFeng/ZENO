...
fn scala_cs_helper_remainder_u8(
    ...
    let tmp_var = FpVar::<Fq>::new_witness(r1cs_core::ns!(cs, "tmp"), || Ok(Fq::zero())).unwrap();
    let mut mul_tmp1 = vec![tmp_var.clone();length];
    let mut mul_tmp2 = vec![tmp_var.clone();length];
    let mut mul_tmp3 = vec![tmp_var.clone();length];
    let mut mul_tmp4 = vec![tmp_var.clone();length];

    for i in 0..input.len() {
        mul_tmp1[i] = constant_mul_cs_helper_u8(cs.clone(), input[i], weight[i]);
        mul_tmp2[i] = constant_mul_cs_helper_u8(cs.clone(), input[i], weight_zeropoint);
        mul_tmp3[i] = constant_mul_constant_cs_helper_u8(cs.clone(), weight[i], input_zeropoint);
        mul_tmp4[i] = constant_mul_constant_cs_helper_u8(cs.clone(), input_zeropoint, weight_zeropoint);
    ...
    tmp1 = tmp1.add_vector(mul_tmp1);
    tmp2 = tmp2.add_vector(mul_tmp2);
    tmp3 = tmp3.add_vector_constant(mul_tmp3);
    tmp4 = tmp4.add_vector_constant(mul_tmp4);
...
fn scala_cs_helper_u8(
    ...
    for i in 0..input.len() {
        ...
        //let w_const = FpVar::new_input(r1cs_core::ns!(cs, "w tmp"), || Ok(w)).unwrap();
        ...
        mul_tmp1[i] = input[i].clone().mul(w_const.clone());
        mul_tmp2[i] = input[i].clone().mul(w0_const.clone());
        mul_tmp3[i] = w_const.clone().mul(input0_const.clone());
        mul_tmp4[i] = w0_const.clone().mul(input0_const.clone());
    }
    tmp1 = tmp1.add_vector(mul_tmp1);
    tmp2 = tmp2.add_vector(mul_tmp2);
    tmp3 = tmp3.add_vector(mul_tmp3);
    tmp4 = tmp4.add_vector_constant(mul_tmp4);
...
fn scala_cs_helper_u8_simd(
    ...
    for i in 0..length {
        ...
        //let w_const = FpVar::new_input(r1cs_core::ns!(cs, "w "), || Ok(w_fq)).unwrap();
        ...
        mul_tmp1[i] = x[i].clone().mul(w_const.clone());
        mul_tmp2[i] = x[i].clone().mul(kernel0_const.clone());
        mul_tmp3[i] = w_const.clone().mul(input0_const.clone());
        mul_tmp4[i] = input0_const.clone().mul(kernel0_const.clone());
    }
    tmp1 = tmp1.add_vector(mul_tmp1);
    tmp2 = tmp2.add_vector(mul_tmp2);
    tmp3 = tmp3.add_vector(mul_tmp3);
