...
use std::thread;
use std::sync::mpsc;
use std::sync::{Arc, RwLock}; //{Arc, Mutex, RwLock}
...
pub struct ConstraintSystem<F: Field> {
    ...
    child_num: Vec<usize>,
    root_tag: Vec<usize>,
    leaf_tag: Arc<RwLock<Vec<usize>>>,
    path_map: Arc<RwLock<Vec<Vec<usize>>>>,
    lc_list:Arc<RwLock<Vec<LinearCombination<F>>>>,
}
...
impl<F: Field> ConstraintSystem<F> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            child_num: Vec::new(),
            root_tag: Vec::new(),
            leaf_tag: Arc::new(RwLock::new(Vec::new())),
            path_map: Arc::new(RwLock::new(Vec::new())),
            lc_list: Arc::new(RwLock::new(Vec::new())),
        }
    }
    ...
    pub fn get_symbolic_lc_num(&self)->usize{
        // self.lc_map.len()
        let mut cnt = 0;
    ...
    pub fn new_lc(&mut self, lc: LinearCombination<F>) -> Result<Variable, SynthesisError> {
        ...
        for i in 0..lc.len(){
            let (_,k) = lc[i];
            if k.is_lc(){
                let target = k.get_lc_index_num();
                self.root_tag[target] += 1;
                child_num += self.child_num[target];
                cnt += 1;
            }
        }
        if cnt == 0{ // have no symbolic LC
            self.root_tag.push(1);
            // self.lc_map.insert(index, lc.clone());
        }
        else{
            self.root_tag.push(0);
        }
        self.child_num.push(child_num+1);
        self.lc_map.insert(index, lc.clone());
        self.lc_list.write().unwrap().push(lc);
    ...
    pub fn print_lc_map(&mut self){
        // println!("\n\ntrace:{:?}",self.constraint_traces);
        println!("map_len:{}",self.lc_map.len());
        println!("lc_map:{:?}\n",self.lc_map);
    }
    ...
    pub fn parallel_inline(&mut self)
    {
        ...
        println!("lc_map len:{}", map_len);

        let thread_num = 8;  // set the thread num
        let set_group_len = map_len / thread_num;
        let mut root_ques:Vec<Vec::<usize>> = vec![Vec::new(); thread_num];
        let mut sort_list = Vec::new();
        for i in 0..self.root_tag.len(){  // get the first level nodes
            if self.root_tag[i] == 0{
                sort_list.push((self.child_num[i],i));
            }
        }
        ...
        for (_,i) in &sort_list{
            let que_idx = cnt%thread_num;
            root_ques[que_idx].push(*i);
            cnt += 1;
        }

        let end1 = Instant::now();
        let prepare_time = end1.duration_since(begin1);
        println!("prepare_time:{:?}",prepare_time);
        
        ///////////////////////////////////////// start compute ///////////////////////////////////////
        let begin2 = Instant::now();
        let mut thread_handlers = Vec::new();
        // let shared_buf = Arc::new(Mutex::new(Vec::with_capacity(thread_num)));
        let (tx, rx) = mpsc::channel();
        for i in 0..thread_num{
            let shared_vec = self.lc_list.clone();
            let shared_que = Arc::new(root_ques[i].clone());
            let arc_i = Arc::new(i.clone());
            let arc_begin = Arc::new(begin2);
            // let buf_local = Arc::clone(&shared_buf);
            let tx = tx.clone();

            let thread_idx = thread::spawn(move || {
                let fp_one:F = 1u32.into();
                let shared_vec = shared_vec.read().unwrap();
                let mut root_que:Vec::<usize> = Vec::new();
                let mut group = Vec::with_capacity(set_group_len);
                let mut new_group = BTreeMap::new();
                let mut used_times = vec![0usize; shared_vec.len()];
                let mut no_lc_tag = vec![0usize; shared_vec.len()];
                for node in &*shared_que{
                    root_que.push(*node);
                }
                while !root_que.is_empty(){
                    let node = root_que.pop().unwrap();
                    let mut cnt = 0;
                    for (_,k) in shared_vec[node].iter(){
                        if k.is_lc(){
                            let target = k.get_lc_index_num();
                            used_times[target]+=1;
                            root_que.push(target);
                            cnt += 1;
                        }
                    }    
                    if cnt >0 {
                ...
                group.sort();
                let end_prepare = Instant::now();
                for &j in &group{
                    let lc = shared_vec[j].clone();
                    let index = LcIndex(j);
                    ...
                    for &(coeff, var) in lc.iter() {
                        if var.is_lc() {
                            let lc_index = var.get_lc_index().expect("should be lc");
                            let target_lc;
                            if no_lc_tag[lc_index.0] == 1{
                                target_lc = &shared_vec[lc_index.0];
                            }
                            else{
                                target_lc = new_group.get(&lc_index).expect("should be inlined");
                            }
                            if coeff == fp_one{
                                inlined_lc.extend(target_lc.clone().0.into_iter());
                            }
                            else{
                                inlined_lc.extend((target_lc * coeff).0.into_iter());
                                tag+=1;
                            }
                            used_times[lc_index.0]-=1;
                            if used_times[lc_index.0] == 0 {
                                new_group.remove(&lc_index);
                                used_times[lc_index.0] = 1;
                            }
                        }
                        else{
                    ...
                    if used_times[index.0] == 0{
                        tx.send((index,inlined_lc.clone()));
                    }
                    new_group.insert(index,inlined_lc);
                }
                tx.send((LcIndex(usize::MAX),LinearCombination::new()));
                let inner_pre = end_prepare.duration_since(*arc_begin);
                let end_com = Instant::now();
                let compute_time = end_com.duration_since(end_prepare);
                // let mut v = buf_local.lock().unwrap();
                // v.push(new_group);
                let end_push = Instant::now();
                let push_time = end_push.duration_since(end_com);
                println!("thread:{:?} finished, inner_prepare_time:{:?},compute_time:{:?},push_time:{:?}",
                *arc_i,inner_pre,compute_time,push_time);
            });
            thread_handlers.push(thread_idx);
        }

        // let buf_unwrap = &*shared_buf.lock().unwrap();

        let mut rx_cnt = 0;
        while rx_cnt < thread_num{
            // let idx_lc = rx.try_recv();
            // let (index,lc) = match idx_lc{
            //     Err(err) => (LcIndex(usize::MAX-1),LinearCombination::new()),
            //     Ok(idx_lcs) => (idx_lcs),
            // };
            // if index.0 == usize::MAX - 1{
            //     continue;
            // }

            let (index,lc) = rx.recv().unwrap();
            if index.0 == usize::MAX{
                rx_cnt += 1;
            }
            else{
                if self.root_tag[index.0] == 0{ 
                    self.lc_map.insert(index, lc.clone());
                }
            }
        }
        ...
        let compute_time = end2.duration_since(begin2);
        
        let begin3 = Instant::now();
        let end3 = Instant::now();
        let insert_time = end3.duration_since(begin3);

        let total_time = end3.duration_since(begin1);
        println!(
            "prepare time:{:?}, parallel compute:{:?}, insert time:{:?}, map_len{:?}",
            prepare_time, compute_time, insert_time, self.lc_map.len()
        );
    ...
    pub fn inline_all_lcs(&mut self) {
        ...
        let mut inlined_lcs:BTreeMap::<LcIndex,LinearCombination<F>>  = BTreeMap::new();
        let mut num_times_used = self.lc_num_times_used(false);
        let compute_time = Duration::from_millis(0);
        let one_coeff:F = 1u32.into();
        for (&index, lc) in &self.lc_map {
            ...
            for &(coeff, var) in lc.iter() {
                if var.is_lc() {
                    ...
                    if coeff == one_coeff{
                        inlined_lc.extend(lc.clone().0.into_iter());
                    }
                    else{
        ...
        println!("total inline time:{:?}", end_total.duration_since(begin_total));
    }
...
impl<F: Field> ConstraintSystemRef<F> {
    ...
    pub fn get_symbolic_lc_num(&self)->usize{
        self.inner()
            .map(|cs| cs.borrow().get_symbolic_lc_num()).unwrap()
    }
    ...
    pub fn print_lc_map(&self) {
        if let Some(cs) = self.inner() {
            cs.borrow_mut().print_lc_map()
        }
    }
