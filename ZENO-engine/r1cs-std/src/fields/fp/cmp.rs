...
impl<F: PrimeField> FpVar<F> {
    ...
    pub fn print_fpvar(&self){
        match self {
            Self::Var(v)=> v.print_fpvar(),
            Self::Constant(c) => println!("Constant:{:?}",c),
        }
    }
    ...
    // This does not create any constraints.
    #[tracing::instrument(target = "r1cs")]
    pub fn add_vector(&self, other: Vec<Self>) -> Self {
        match self {
            Self::Var(v)=> Self::Var(v.add_vector(other)),
    ...
    // This does not create any constraints.
    #[tracing::instrument(target = "r1cs")]
    pub fn add_vector_constant(&self, other: Vec<Self>) -> Self {
        match self {
            Self::Var(v)=> Self::Var(v.add_vector_constant(other)),
            Self::Constant(c) => Self::Constant(*c),
        }
    }
    ...
    pub fn variable(&self) -> Variable{
        match self {
            Self::Constant(v) => Variable::Zero,
            Self::Var(v) => v.variable,
        }
    }
