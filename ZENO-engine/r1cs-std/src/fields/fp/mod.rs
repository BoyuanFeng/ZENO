...
impl<F: PrimeField> AllocatedFp<F> {
    ...
    pub fn print_fpvar(&self){
        println!("value:{:?},variable:{:?}",self.value,self.variable);
    }

    #[tracing::instrument(target = "r1cs")]
    pub fn get_value(&self)->F{
        let fp_one:F = 1u128.into();
        match self.value{
            Some(val) => val,
            None => fp_one,
        }
    ...
    /// This does not create any constraints.
    #[tracing::instrument(target = "r1cs")]
    pub fn add_vector(&self, other: Vec<FpVar<F>>) -> Self {
        // println!("add_vector");
        let mut value = self.value;
        let mut variable_vec = LinearCombination::<F>::new();
        let fp_one:F = 1u128.into();
        for i in 0..other.len(){
            value = match value{
                Some(val) => Some(val + other[i].get_value()),
                None => None,
            };
            variable_vec.push((fp_one,other[i].variable()));
        };
        let variable = self
            .cs
            .new_lc(lc!() + self.variable + variable_vec)
            .unwrap();
        AllocatedFp::new(value, variable, self.cs.clone())
        // let k = AllocatedFp::new(value, variable, self.cs.clone());
        // k.print_fpvar();
        // k
    }
    ...
    /// This does not create any constraints.
    #[tracing::instrument(target = "r1cs")]
    pub fn add_vector_constant(&self, other: Vec<FpVar<F>>) -> Self {
        // println!("add_vector_constant");
        let mut value = self.value;
        let mut variable_vec = LinearCombination::<F>::new();
        for i in 0..other.len(){
            if !other[i].get_value().is_zero(){
                value = match value{
                    Some(val) => Some(val + other[i].get_value()),
                    None => None,
                };
                variable_vec.push((other[i].get_value(), Variable::One));
            }
        }
        if variable_vec.len() == 0 {
            self.clone()
        }
        else{
            let variable = self
